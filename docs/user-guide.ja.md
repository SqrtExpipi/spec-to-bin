...
| テスト一式ZIP | テンプレート、生成BIN、ハッシュ、生成情報をまとめて保存します。 |
| JSONファイルを開く | 既存のテンプレートJSONを読み込み、現在の表を置き換えます。未保存の変更がある場合は確認します。 |
| JSONを直接編集 | ツールバー直下にJSON編集領域を開きます。編集後は**JSONを反映**します。 |
...
| 保存時点に戻す | 最後にJSONから開いた、またはJSONへ保存した定義へ戻します。この操作自体はUndoできます。 |
| 新規作成 | 現在の定義を空の新規テンプレートへ置き換えます。 |
...
行をクリックすると、その行がHex強調表示と貼り付け位置に使う**アクティブ行**になります。左端のチェックボックスは、それとは独立した**一括操作の選択**です。一括操作バーは行がある間は表示され、ページをスクロールしても上部付近へ追従します。

選択行の複製、エディタ内クリップボードへのコピー、上下移動、削除、および選択範囲を1レコードとした繰り返し展開ができます。**貼り付け**はクリップボード内の行をアクティブ行の直下へ追加します。一括削除直後は通知からUndoできます。
...
**テスト値を生成**は、選択したロックされていない固定長`string`項目だけに適用します。各項目の実効EncodingとSizeをbyte単位で使用します。A-Zを最大長まで繰り返す、全角最大、現在値を残して補完、指定文字、数字、空文字、末尾1 Byte空けを選べます。ダイアログには対象数、除外数、各行の除外理由を表示します。全角文字だけで長さが一致しない場合は、半角で補完、最大長未満、スキップから選択します。
...
JSON直接編集では、JSONを貼り付けた後に**JSONを反映**するまで表へは反映されません。編集領域は開いている間、メインツールバーの直下に表示されます。JSON構文エラーは解消するまで表示され、ブラウザから解析位置を取得できる場合は行・列も示します。編集内容を反映するまではJSONファイル保存が無効になります。
